...
# The Chrome extension runs on a real page (mail.google.com, app.slack.com) and
# calls this backend from a content script, so the browser sends a preflight
# and requires an explicit allow. Only extension origins are allowed, and only
# on localhost — this is a single-user local application, not a public API.
...
def api_memory():
    ...
    try:
        ...
        out = {"fact": [], "preference": [], "episode": [], "shortcut": []}
        for r in rows:
            if r["kind"] == "episode":
                continue
            if r["kind"] == "shortcut":
                p = jload(r["payload"])
                out["shortcut"].append({
                    "id": r["id"], "phrase": p.get("phrase", r["subject"]),
                    "body": r["body"], "understood": p.get("understood", []),
                    "apps": p.get("apps", []), "fires_on": p.get("fires_on", "selection"),
                    "use_count": r["use_count"], "status": r["status"],
        ...
        counts = {
            "shortcuts": conn.execute(
                "SELECT COUNT(*) c FROM memory WHERE kind='shortcut' AND status='active'"
            ).fetchone()["c"],
            "facts": len(out["fact"]),
...
def api_shortcuts():
    ...
    try:
        return jsonify({
            "shortcuts": shortcuts.listing(conn),
            "collisions": shortcuts.collisions(conn),
        })
    finally:
        conn.close()
...
def api_shortcut_interpret():
    """Read a definition back without saving it. Interpretation is not
    permission: the person confirms the reading before it becomes real."""
    p = request.get_json(force=True) or {}
    return jsonify(shortcuts.interpret(
        p.get("phrase", ""), p.get("meaning", ""),
        apps=p.get("apps") or [], fires_on=p.get("fires_on", "selection"),
...
def api_shortcut_save():
    ...
    draft = p.get("draft")
    if not draft:
        # Allow saving straight from an edited definition too.
        draft = shortcuts.interpret(p.get("phrase", ""), p.get("meaning", ""),
                                     apps=p.get("apps") or [],
                                     fires_on=p.get("fires_on", "selection"))
    conn = db()
    try:
        return jsonify(shortcuts.save(conn, draft))
    finally:
        conn.close()


@app.post("/api/shortcuts/<mid>/rename")
def api_shortcut_rename(mid: str):
    p = request.get_json(force=True) or {}
    conn = db()
    try:
        return jsonify(shortcuts.rename(conn, mid, p.get("phrase", "")))
    finally:
        conn.close()
...
def api_seed():
    ...
    try:
        ...
        for rec in corpus_io.load(CORPUS_PATH):
            dictate(conn, rec)
            shortcuts.install_from_record(conn, rec)
            n += 1
