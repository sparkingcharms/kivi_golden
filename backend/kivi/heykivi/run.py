...
# The brief's own example is two requests in one sentence: find something,
# then change it. Chaining is not a bonus feature, it is the shape of the
# use case.
_CHAIN = re.compile(
...
def dictate(conn: sqlite3.Connection, record: dict[str, Any]) -> dict[str, Any]:
    """One dictation pass: correct known spellings, write it, learn from it."""
    ...
    trace.step("dictionary", replaced=[u["body"] for u in used],
               changed=corrected != text)

    record = {**record, "formatted": corrected}
    report = store.ingest(conn, record)
    trace.step("learn",
               created=[c["body"][:80] for c in report["created"]],
               reinforced=[c["body"][:80] for c in report["reinforced"]],
               conflicts=[c["body"][:80] for c in report["conflicts"]],
    ...
    return {
        "text": corrected,
        "was": text if corrected != text else None,
        "corrections": used,
        "learned": report,
        "trace": info,
    }
...
def ask(conn: sqlite3.Connection, request: str, app: str | None = None,
    ...
    # --- teaching a shortcut ------------------------------------------------
    # "professorize this means formal, three sentences, one clear ask."
    # This returns a draft. Nothing is stored: the person reads back what Kivi
    # understood and confirms it, the same principle as the promotion gate
    # applied at the moment of teaching.
    taught = shortcuts.parse_teaching(request)
    if taught:
        draft = shortcuts.interpret(taught["phrase"], taught["meaning"],
        ...
        return {
            "status": "shortcut_draft",
            "say": f"Here is how I read “{draft['phrase']}”. Correct anything, then save it.",
            "draft": draft,
            "intent": "teach_shortcut",
            "trace": info,
        }

    # --- firing a shortcut --------------------------------------------------
    # Checked before intent classification, because a phrase the person
    # invented is not one of Kivi's six intents and should not be forced into
    # them. If nothing matches, this returns None and routing proceeds.
    ...
    if hit:
        ...
        if hit["status"] == "matched":
            ...
            # A near miss that Kivi went ahead with anyway still needs saying,
            # or the person only finds out from a message they already sent.
    ...
    outcome = {
        "found": "answered", "found_and_reshaped": "answered", "drafted": "answered",
        "answered": "answered", "remembered": "answered", "forgotten": "answered",
        "ambiguous": "asked", "unclear": "asked", "not_found": "none",
        "ambiguous_trigger": "asked", "out_of_scope": "asked",
        "needs_selection": "asked", "shortcut_draft": "asked",
        "nothing_known": "none", "declined": "declined",
    }.get(result.get("status", ""), "answered")
...
_INTENT_LABELS = {
    "find_dictation": "find something you dictated",
    "reshape": "rewrite what is selected",
    "recall": "tell you what Kivi knows",
    "draft_message": "draft a message",
    "remember": "remember a rule",
    "forget": "drop a rule",
}


def _dispatch(conn, intent, request, app, selection, backend, trace) -> dict[str, Any]:
    if intent == "find_dictation":
        return tools.find_dictation(conn, request, trace)
    if intent == "reshape":
        if not selection:
    ...
    if intent == "recall":
        return tools.recall(conn, request, trace)
    if intent == "draft_message":
        return tools.draft_message(conn, request, app, backend, trace)
    if intent == "remember":
        return tools.remember(conn, request, trace)
    if intent == "forget":
        return tools.forget(conn, request, trace)
