"""HTTP surface for the local Kivi product."""
...
def api_memory():
    ...
    try:
        ...
        eps=conn.execute("SELECT * FROM memory WHERE kind='episode' AND status='active' ORDER BY created_ts DESC LIMIT 25").fetchall();out["episode"]=[{"id":r["id"],"body":r["body"],"app":r["scope_app"],"created_ts":r["created_ts"],"source_utt":r["source_utt"]} for r in eps]
        counts={"shortcuts":len(out["shortcut"]),"facts":len(out["fact"]),"rules_active":sum(1 for p in out["preference"] if p["status"]=="active"),"rules_observed":sum(1 for p in out["preference"] if p["status"]=="observed"),"episodes":len(eps),"db_bytes":db_size_bytes(DB_PATH)}
...
def api_shortcut_save():
    p=request.get_json(force=True) or {};draft=p.get("draft")
    if not draft:draft=shortcuts.interpret(p.get("phrase",""),p.get("meaning",""),apps=p.get("apps") or [],fires_on=p.get("fires_on","selection"))
    conn=db()
