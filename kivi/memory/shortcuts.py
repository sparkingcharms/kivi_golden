"""Fourth memory kind: user-defined phrases bound to reviewed instructions."""
from __future__ import annotations
import re, sqlite3, uuid
from difflib import SequenceMatcher
from typing import Any
from ..db import jdump,jload,now
from ..local_model import _compile_rule,content_tokens,normalise_key
COLLISION_MARGIN=.18; TRIGGER_FLOOR=.75
DEICTIC={"this","that","it","these","those","them","here","mine","my","the","a","an"}
FIRES_ON=("selection","voice","any")
_TEACH=re.compile(r'^\s*(?:hey kivi[\s,]*)?(?:teach (?:me )?a shortcut[,:]?\s*)?[\"“\']?(?P<phrase>[\w\' -]{2,40}?)[\"”\']?\s+(?:means|should mean|means that|=)\s+(?P<meaning>.{6,300})$',re.I)
def _id(): return f"mem_{uuid.uuid4().hex[:12]}"
def phrase_similarity(a,b): return SequenceMatcher(None,(a or '').strip().lower(),(b or '').strip().lower()).ratio()
def parse_teaching(text):
 m=_TEACH.search(text or '')
 if not m:return None
 phrase=m.group('phrase').strip().strip('"\'“”');meaning=m.group('meaning').strip().rstrip('.')
 return None if len(phrase.split())>5 or not meaning else {'phrase':phrase,'meaning':meaning}
def _split_clauses(meaning):
 parts=re.split(r'\s*[,;. ]\s*',meaning)
 # Preserve ordinary spaces inside clauses: split only at punctuation.
 parts=re.split(r'\s*[,;.]+\s*',meaning)
 out=[]
 for p in parts:
  p=p.strip(' .,;')
...
def _line_is_structured(line,rules):
 low=line.lower();return any(k in rules and any(w in low for w in ws) for k,ws in _RULE_WORDS.items())
def interpret(phrase,meaning,apps=None,fires_on='selection',examples=None):
 lines=_split_clauses(meaning);rules=_compile_rule(meaning);understood=[{'n':i+1,'text':x,'structured':_line_is_structured(x,rules)} for i,x in enumerate(lines)]
 return {'phrase':phrase,'trigger':normalise_key(phrase),'meaning':meaning,'understood':understood,'rules':rules,'apps':apps or [],'fires_on':fires_on if fires_on in FIRES_ON else 'selection','examples':examples or [],'not_executable':[x['text'] for x in understood if not x['structured']],'status':'draft'}
def _all(conn):
 ...
 return out
def _nearest(conn,trigger,exclude=None):
 best=None
 for s in _all(conn):
 ...
 return best
def save(conn,draft,utt_id=None):
 ...
 near=_nearest(conn,trigger);mid=_id();payload={'trigger':trigger,'phrase':draft['phrase'],'meaning':draft['meaning'],'understood':[u['text'] for u in draft.get('understood',[])],'rules':draft.get('rules',{}),'apps':draft.get('apps',[]),'fires_on':draft.get('fires_on','selection'),'examples':draft.get('examples',[]),'not_executable':draft.get('not_executable',[])};t=now()
 conn.execute("INSERT INTO memory (id,kind,subject,body,payload,status,confidence,observations,scope_app,created_ts,updated_ts,source_utt,origin,reason) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(mid,'shortcut',trigger,draft['meaning'],jdump(payload),'active',.97,1,(draft.get('apps') or [None])[0],t,t,utt_id,'taught','you defined this phrase yourself'))
 conn.execute("INSERT INTO memory_event (memory_id,ts,event,detail,utt_id) VALUES (?,?,?,?,?)",(mid,t,'promoted','taught and confirmed',utt_id));conn.commit();out={'status':'saved','id':mid,'phrase':draft['phrase'],'trigger':trigger,'fires_on':payload['fires_on'],'apps':payload['apps']}
...
def _head_similarity(said,trigger):
 heads=[t for t in content_tokens(trigger) if t not in DEICTIC] or content_tokens(trigger);toks=content_tokens(said) or [said];grams=list(toks)+[f'{toks[i]} {toks[i+1]}' for i in range(len(toks)-1)]+[f'{toks[i]}-{toks[i+1]}' for i in range(len(toks)-1)];return max((phrase_similarity(h,g) for h in heads for g in grams),default=0)
def match(conn,said,app,has_selection,trace=None):
 ss=_all(conn)
 if not ss:return None
 key=normalise_key(said);scored=[]
 for s in ss:
  trig=s['trigger'];verbatim=trig in key;head=_head_similarity(said,trig);score=1 if verbatim else (max(head,phrase_similarity(key,trig)) if head>=TRIGGER_FLOOR else head);scored.append({**s,'score':round(score,4),'verbatim':verbatim})
...
def collisions(conn):
 rows=conn.execute("SELECT c.*,a.body chosen_body,b.body against_body FROM shortcut_collision c LEFT JOIN memory a ON a.id=c.chosen LEFT JOIN memory b ON b.id=c.against WHERE c.resolved=0 ORDER BY c.ts DESC LIMIT 10").fetchall();return [{'id':r['id'],'said':r['said'],'margin':r['margin'],'chosen':r['chosen'],'against':r['against'],'chosen_body':r['chosen_body'],'against_body':r['against_body']} for r in rows]
...
def rename(conn,memory_id,phrase):
 ...
 payload=jload(row['payload']);payload['phrase']=phrase;payload['trigger']=normalise_key(phrase);conn.execute('UPDATE memory SET subject=?,payload=?,updated_ts=? WHERE id=?',(payload['trigger'],jdump(payload),now(),memory_id));conn.execute('UPDATE shortcut_collision SET resolved=1 WHERE chosen=? OR against=?',(memory_id,memory_id));conn.execute('INSERT INTO memory_event (memory_id,ts,event,detail) VALUES (?,?,?,?)',(memory_id,now(),'edited',f'renamed to “{phrase}”'));conn.commit();return {'status':'renamed','id':memory_id,'phrase':phrase,'trigger':payload['trigger']}
def install_from_record(conn,record):
 ...
 return save(conn,interpret(spec['phrase'],spec.get('meaning',''),apps=spec.get('apps') or [],fires_on=spec.get('fires_on','selection')),utt_id=record.get('id'))
def listing(conn):
